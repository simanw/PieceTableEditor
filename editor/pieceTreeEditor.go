...
type MyPieceTreeEditor struct {
	OriginalFile string
	Pt           model.PieceTable_Tree
	Trie         *model.Trie
	Clipboard    *[]byte
}

func (e *MyPieceTreeEditor) InitMyPieceTreeEditor(file string) {
	...
	e.OriginalFile = file
	e.Pt = model.NewPieceTable(file)
	e.Clipboard = &[]byte{}
}

func (e *MyPieceTreeEditor) Cut(i, j int) {
	//fmt.Println("output table in Cut() ", e.Pt.Table)
	e.Clipboard = e.Pt.StringAt(i, j-i)
	e.Pt.Delete(i, j-i)
}

func (e *MyPieceTreeEditor) Copy(i, j int) {
	e.Clipboard = e.Pt.StringAt(i, j-i)
	//fmt.Println("Copying clipboard", e.Clipboard)
}

func (e *MyPieceTreeEditor) Paste(i int) {
...
func (e *MyPieceTreeEditor) GetText() string {
	return e.Pt.GetFile()
}

func (e *MyPieceTreeEditor) Misspellings() int {
	result := 0
...
func (e *MyPieceTreeEditor) Insert(str string, i int) {
	e.Pt.Insert([]byte(str), i)
}

func (e *MyPieceTreeEditor) Delete(i, j int) {
	e.Pt.Delete(i, j-i)
}
