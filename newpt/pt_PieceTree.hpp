...
#ifndef PT_PIECETREE_H
#define PT_PIECETREE_H
...
public:
	...
	private:
		Iterator(pt_PieceTree* owner, Node* node = 0) : m_pOwner(owner), m_pNode(node) {}
		const Node* getNode() const { return m_pNode; }
		Node* getNode() { return m_pNode; }

		pt_PieceTree* m_pOwner;
		Node* m_pNode;
		friend class pt_PieceTree;
	};
	...
	pd_DocPosition sizeDocument() const { return m_nDocumentSize; }

#ifdef DEBUG
	void print() const;
	bool checkInvariants() const;
	bool checkSizeInvariant(const Node* pn, pd_DocPosition* size) const;
#endif

private:
	pt_PieceTree(const pt_PieceTree&);
	pt_PieceTree& operator= (const pt_PieceTree&);
	...
	pd_DocPosition _calculateSize(Node* x);
	...
	Node* m_pRoot;
	size_t m_nSize;
	pd_DocPosition m_nDocumentSize;

	friend class Iterator;
};

#endif // PT_PIECETREE_H
